...
const OTPVerificationModal = () => {
  ...
  const handleOtpChange = (index, value) => {
    ...
    next[index] = d;
    setOtp(next);
    if (d && index < 5) document.getElementById(`fe-otp-signin-${index + 1}`)?.focus();
  };

  const handleKeyDown = (index, e) => {
    if (e.key === 'Backspace' && !otp[index] && index > 0) {
      document.getElementById(`fe-otp-signin-${index - 1}`)?.focus();
    }
  };
      ...
      <div className='fe-modal-content'>
        <div className='fe-modal-header'>
          ...
          <div className='fe-modal-text'>
            <LuShield className='fe-modal-shield-icon' /> Log In
          </div>
        </div>
        <div className='fe-modal-otp-container'>
          ...
          <p className='fe-otp-timer'>
            {resendSec > 0 ? (
              ...
              <span className='fe-resend-link' onClick={handleResend}>
                Resend OTP
              </span>
            )}
          </p>
